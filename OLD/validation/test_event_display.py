...
from analysis import format_event_details
import run_event_display
from OLD.validation.fixtures import EventChain, base_event, vector
...
class EventDisplayTests(unittest.TestCase):
    def test_xy_and_rz_include_every_track_and_cluster(self):
        ...
        with TemporaryDirectory() as directory, \
                patch("histograms.load_root", return_value=graphics.root), \
                patch("histograms.root_output", return_value=nullcontext()):
    ...
    def test_dedicated_runner_selects_display_and_saves_event_details(self):
        ...
        with TemporaryDirectory() as directory:
            ...
            with patch("analysis_io.build_chain", return_value=chain), \
                    patch("analysis_io.load_root", return_value=NS(gROOT=NS(GetVersion=lambda: "test"))), \
                    patch("run_event_display.draw_event_display", return_value=files) as draw, \
                    patch("run_analysis.ElectronAnalysis", side_effect=AssertionError("Wrong workflow")), \
                    redirect_stdout(stdout):
                run_event_display.main([str(input_path), "--event", "1", "--output", str(output)])
