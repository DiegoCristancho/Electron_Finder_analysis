...
from electron_finder.io import iter_events
from electron_finder.reporting import format_analysis_summary
from run_analysis import make_parser
from tests.fixtures import EventChain, selection_cases
from tests.reference import PROJECT, assigns, run_reference
...
class ReportingAndSelectionTests(unittest.TestCase):
    ...
    def test_cli_supports_event_one_and_rejects_max_events_together(self):
        parser = make_parser()
        args = parser.parse_args(["analyze", "--input", "sample.root", "--event", "1"])
        ...
        with patch("sys.stderr", new=io.StringIO()), self.assertRaises(SystemExit) as error:
            parser.parse_args(["analyze", "--input", "sample.root", "--event", "1", "--max-events", "10"])
